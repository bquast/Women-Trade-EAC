...
k13_pr <- formula(female_share_prod    ~ international  + capital_city + multi_establ + intern_certif + industry + country)
k13_np <- formula(female_share_nonprod ~ international  + capital_city + multi_establ + intern_certif + industry + country)
...
k13_pr_r1 <-  lm(k13_pr, data = merge13)
k13_pr_r2 <- glm(k13_pr, data = merge13, family = quasibinomial(link='logit') )
k13_np_r1 <-  lm(k13_np, data = merge13)
k13_np_r2 <- glm(k13_np, data = merge13, family = quasibinomial(link='logit') )
